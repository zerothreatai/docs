...
User roles are essential for defining the scope of access and actions each user can perform within the organization in the ZeroThreat portal. By assigning roles effectively, organizations can maintain security, ensure accountability, and streamline collaboration across teams. Each role is tailored to specific levels of responsibility and access, ensuring that users only have the permissions necessary for their tasks. Let’s explore the roles available and their capabilities in detail.
...
- **Example**: An Owner might configure a target, update the organization's subscription plan, and assign roles to new team members.
...
###### ZeroThreat advises assigning roles based on the principle of least privilege to minimize the risk of unauthorized changes or data exposure.
...
Understood about User Roles? Now head over to our [**Add User**](add-user 'mention') guide to add a user to your organization.

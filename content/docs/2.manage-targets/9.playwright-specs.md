...
For more details, refer to the official **Playwright Test Agents** documentation. After generating the spec, run it and check whether it passes.

```bash
npx playwright test
```

If the spec fails because of broken selectors, timing issues, navigation mismatches, or environment-specific behavior, pass it to the **Healer** to repair those issues.

The healer is useful for:

* fixing unstable selectors
* correcting waits or assertions
* adjusting steps when the generated flow is slightly off
* improving reliability of the spec

If the spec still does not work after healing, use **manual intervention** to finish the cleanup. In practice, the best results usually come from combining agent help with a quick manual review, especially for dynamic or stateful applications.

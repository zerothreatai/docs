...
* You’re on the Enterprise plan in ZeroThreat, or any other plan that includes Notification Integrations (The target must also be associated with the subscription). Check the [**ZeroThreat Plans**](/docs/plans/zerothreat-plans) page for more details.
...
2. Inside Workflows, you will find popular Teams templates. Here, select the **“Post to a channel when a webhook request is received”** template.
...
5. This will generate a **Webhook URL** which you'll need to connect with ZeroThreat. After the webhook is created, click on **Copy URL** to save it to your clipboard. This will be used in the next step. Click **Done** to finish the workflow setup.
...
1. Go to the **Targets** section in ZeroThreat and select the **Target Configuration** (<img src="/Images/image (78).png" alt="Select Target Image">) icon for the required target.
...
ZeroThreat will validate the webhook. If successful, you’ll receive a message in your Teams channel confirming a successful integration.

::fiqure-img{source="/Images/image (313).png"}
::

Once integrated, you’ll start receiving Teams messages in the selected channel whenever a scan starts or completes. These alerts help teams quickly respond to scan results and coordinate more efficiently around detected vulnerabilities.

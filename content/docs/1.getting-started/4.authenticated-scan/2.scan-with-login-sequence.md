...
::hint{ bgStyle="bg-yellow-50 rounded-md border-yellow-200 text-yellow-600" icon="triangle-exclamation" iconClass="text-yellow-400"}
...
::fiqure-img{source="/image (97).png"}
...
::hint{bgStyle=" bg-red-100 rounded-md border-red-300 text-red-500" icon="triangle-exclamation" iconClass="text-red-300"}
...
* Once you have navigated few authenticated sections, log out of the target application.<img src="/image (102).png" width="17%" alt="" style="display:inline"/>


* Click on **"Stop Recording"** <img src="/image (103).png" alt="" width="120px" data-size="line" style="display:inline"> to stop the recording.

### 7. Tag the HTTP Requests

After stopping the recording, you'll need to tag the captured HTTP requests:

* **Login Action**: Tag the HTTP request responsible for logging in.&#x20;
...
* **Authenticated Requests**: Tag any HTTP requests that is only accessible after login.
...
* **Log Out Action**: Tag the HTTP request responsible for logging out.

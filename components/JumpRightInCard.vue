<script setup>// Get frontmatter data
...
<template>
  <div class="">
    <div v-if="cards.length" class="flex gap-9">
      <div v-for="(card, index) in cards" :key="index"  class='border border-2 rounded justify-center hover:text-purple-500  grid p-1  min-w-[200px] hover:bg-slate-100 '>
        <NuxtLink :to="card.path" class="">
            <h5>
                {{ card.title }}
            </h5>
          <span>

<template>
  <div class="max-h-[calc(100vh-125px)] overflow-y-scroll pb-10 z-10">
    <div v-for="(items, index) in props.navigation" :key="index">
      <h5 v-if="items.children" class="text-xs font-zt_bold tracking-wide uppercase text-gray-800 pt-4 pb-2">
        {{ items.title }}
      </h5>
      <NuxtLink v-else class="font-zt_medium tracking-wide text-gray-500 hover:text-zt_purple" :class="{'text-zt_purple' : route.path === items.path }" :to="items.path"> 
